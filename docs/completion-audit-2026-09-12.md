...
| IaC pipeline implemented | GitHub workflow has PR/main validation, manual protected deployment, explicit OIDC account restriction, account verification, and corrected repository-root paths | PROVADO as code; runtime roles/variables not configured, and the current `AWSPowerUserAccess` session cannot inspect IAM roles (`iam:GetRole` denied) |
