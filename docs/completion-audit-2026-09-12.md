...
| IaC pipeline implemented | GitHub workflow has PR/main validation, fork-safe AWS preview behavior, explicit OIDC account restriction, account verification, preview-only manual dispatch by default, main-only deployment dispatch, and corrected repository-root paths. `SkyXIacPreviewRole` exists with its read-only policy. | PROVADO as code and preview-role setup; repository variable is configured, while deploy role/environment remain unconfigured |
