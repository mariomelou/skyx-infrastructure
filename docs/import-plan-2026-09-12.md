...
| ECS task definitions | `skyx-api:5` and `skyx-frontend:1` | Treat revisions as application-owned artifacts; reference the active revision from infrastructure rather than recreate it | High. Never replace a running task definition implicitly; backend retains migration ownership |
