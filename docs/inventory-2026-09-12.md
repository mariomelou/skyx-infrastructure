...
| ECS | Cluster `skyx-prod`; services `skyx-api` and `skyx-frontend`; one desired/running Fargate task each; task definitions `skyx-api:5` and `skyx-frontend:1`; availability-zone rebalancing on; auto-assign public IP on; no autoscaling, Service Connect, or service discovery configured. |
