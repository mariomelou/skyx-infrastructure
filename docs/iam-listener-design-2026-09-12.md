...
The scoped deployment-role grant is `PROVADO`, and the authorized backend rerun successfully completed migrations and ECS rollout. The previously observed wildcard Cognito resource was corrected through the authorized AWS Console change on 2026-09-15. The current policy now trusts `ecs-tasks.amazonaws.com`, includes `cognito-idp:AdminCreateUser` and `cognito-idp:AdminGetUser`, and limits `Resource` to `arn:aws:cognito-idp:us-east-1:129346407469:userpool/us-east-1_wiOmAOPbu`. The Cognito policy-scope and deployment-path gates are now `PROVADO`; invitation/RBAC/tenant-isolation/access-revocation validation remains pending. If console deployment is required, update the IAM Identity Center permission set instead.
