...
| IAM | SkyX roles observed: `SkyXCodeBuildRole`, `SkyXEcsTaskExecutionRole`, and `SkyXGitHubActionsEcrRole`. The ECS execution role trusts `ecs-tasks.amazonaws.com` and has AWS-managed `AmazonECSTaskExecutionRolePolicy` plus inline `SkyXFrontendSecretRead` and `SkyXSecretsRead`. The GitHub role trusts `token.actions.githubusercontent.com` with audience `sts.amazonaws.com` and repository subject conditions; its inline policies are `SkyXEcrPush` and `SkyxEcsMigrationRunTask`. Existing roles are referenced in the adoption plan; they are not recreated. CodeBuild policy details remain pending. |
...
The following are deliberately `PENDING` or `UNVERIFIED`: ALB attributes beyond the listener-level response-header/server-header view, CodeBuild policy and trust documents, endpoint policy details for resources outside the SkyX VPC, and the complete resource-by-resource import specification. These require another bounded read-only pass or an approved ownership design before adoption is modeled as authoritative.
