...
3. **Import design (defined; first wave scoped, approval pending).** The resource-by-resource treatment, dependency order, risk gates, and rollback boundary are documented in [import-plan-2026-09-12.md](import-plan-2026-09-12.md). The proposed first wave is the one-repository-at-a-time ECR procedure in [adoption-wave-1-ecr-2026-09-12.md](adoption-wave-1-ecr-2026-09-12.md), currently recommended for `skyx-backend`. For each resource, choose one of: retain external ownership, import into a dedicated CloudFormation stack, or model with a CDK reference. Import must be resource-specific and tested against a snapshot. No broad replacement is acceptable.
4. **Staged ownership (approval required).** Import low-risk supporting resources first, then review `cdk diff` and CloudFormation change sets. Keep database, secrets, Cognito, and live ECS services protected with retain policies and explicit rollback procedures. Hardening remains separately designed in [hardening-plan-2026-09-12.md](hardening-plan-2026-09-12.md).
