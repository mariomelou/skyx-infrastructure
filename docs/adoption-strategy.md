...
2. **Complete read-only inventory (in progress).** SG rules, route entries, service counts, ECS deployment behavior, listener/rule state, ALB attributes, alarms, WAF absence, Config state, endpoint placement, and IAM policy/trust documents are recorded. The remaining read-only gaps are target-group health-check details and endpoint-policy details for resources outside the SkyX VPC.
