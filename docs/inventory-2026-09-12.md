...
| Load balancing | `skyx-prod-alb` targets API port `8000`; its listener is HTTP:80 with one rule and no TLS certificate. `skyx-prod-frontend-alb` targets frontend port `3001`; its listener is also HTTP:80 with one rule and no TLS certificate. Both are internet-facing and in the SkyX VPC. |
...
The following are deliberately `PENDING` or `UNVERIFIED`: listener rule match conditions and ALB attributes beyond the listener summary, ECS deployment circuit-breaker details, IAM policy documents, and the complete frontend task-definition image metadata. These require another bounded read-only pass before ownership is modeled as authoritative.
