...
| Load balancing | `skyx-prod-alb` targets API port `8000`; its listener was observed as HTTP port `80` with one rule and no TLS certificate. `skyx-prod-frontend-alb` targets frontend port `3001`; its listener details remain pending. Both are internet-facing and in the SkyX VPC. |
...
The following are deliberately `PENDING` or `UNVERIFIED`: the frontend ALB listener/rule/TLS configuration, all security-group rule directions, route entries, endpoint identity and policy, ECS deployment circuit-breaker/autoscaling details, CloudWatch alarms/metrics, Config recorder/delivery settings, WAF association state, IAM policy documents, and the complete frontend task-definition image metadata. These require another bounded read-only pass before ownership is modeled as authoritative.
